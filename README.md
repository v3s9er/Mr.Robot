...
> 현재 릴리스: **0.4.5**. 빠른 사용법은 [docs/USER_GUIDE_0.3.md](docs/USER_GUIDE_0.3.md), 변경 사항은 [docs/RELEASE_NOTES_0.4.5.md](docs/RELEASE_NOTES_0.4.5.md), 새 플러그인의 독립 구현·기능 비교는 [docs/PLUGIN_COMPARISON_0.4.2.md](docs/PLUGIN_COMPARISON_0.4.2.md), 설계·연구·라이선스 근거는 [docs/RESEARCH_AND_LICENSES.md](docs/RESEARCH_AND_LICENSES.md)를 먼저 보세요.
...
- ◈ **대화별 토큰 정책** — 권한 옆에서 64K/192K/512K 기준과 실행당 8M 절대 상한을 쓰는 품질 우선 적응형 또는 내장 네이티브 앱의 로컬 관리자용 무제한 감사 모드를 선택하고 사용량은 계속 기록
...
결과는 `release/Mr.Robot-Setup-0.4.5-x64.exe`입니다. 현재 빌드는 개발용 미서명 설치 파일이므로 조직의 Windows 애플리케이션 제어 정책에서 차단될 수 있습니다. 공개 배포본에는 코드 서명 인증서를 적용해야 합니다.
...
설정 → 모델 라우팅에서 LangChain식 캔버스에 입력·분류·모델·검증·메모리·응답 노드를 자유 배치하고 연결합니다. 절약·균형·품질·코딩 기본 의사결정 트리를 고르거나 현재 트리를 사용자 프리셋으로 저장해 목록에서 다시 적용할 수 있습니다. 모델 노드의 역할과 제공자를 선택하면 그래프 순서가 실제 역할별 우선순위가 됩니다. 대화에서 특정 모델을 고르면 자동 라우팅보다 우선합니다. 추론 강도는 데스크톱과 모바일 모두 입력창 하단에서 자동/없음/낮음/보통/높음/매우 높음/최대 중 현재 제공자가 지원하는 값만 선택하며 대화마다 저장됩니다. 권한 바로 옆의 토큰 정책은 기본 `적응형 · 품질 우선`과 내장 네이티브 앱의 로컬 관리자 전용 `무제한 · 감사만` 중 선택합니다. 네이티브 메인 프로세스와 내장 서버 사이의 짧은 1회성 증명까지 확인하므로 일반 브라우저나 localhost로 위장한 프록시는 이 권한을 얻지 못합니다. 적응형은 단순 64K, 표준 192K, 복합 512K 기준을 품질 신호에 따라 확장하되 실행당 8M 절대 상한을 둡니다. 무제한 감사 모드는 Mr.Robot의 누적 토큰 예산으로 작업을 중단하지 않지만 공급자 호출 수·시간·스트림·전송·동시 실행·최대 단계·무진전 반복 안전장치와 공급자 자체 한도·요금은 유지합니다. 원격·연결 기기·REST·가져온 대화는 적응형으로 고정됩니다. 같은 화면에서 공급자가 보고한 실제 입력·출력·예상 비용과 예약량·실패 중 부분 사용량까지 보수적으로 기록한 `감사 토큰`, 도구 호출·실패·지연 통계를 구분해 확인합니다.
