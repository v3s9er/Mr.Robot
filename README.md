...
결과는 `release/Mr.Robot-Setup-0.4.8-x64.exe`입니다. 현재 빌드는 개발용 미서명 설치 파일이므로 조직의 Windows 애플리케이션 제어 정책에서 차단될 수 있습니다. 공개 배포본에는 코드 서명 인증서를 적용해야 합니다.
...
[Mr. Robot Page Publisher](plugins/mr-robot/README.md)는 페이지 저장·수정 이력·복원·로컬 미리보기·Cloudflare Worker 패키징 모듈입니다. 데스크톱 앱의 개발 카테고리에 기본 OFF로 장착되며, 켜짐/꺼짐은 재시작 후에도 유지됩니다. 공개 저장소에는 서버 측 어댑터·도구·사용 지침만 포함하며, 사용자 페이지 HTML/CSS/JavaScript와 생성된 배포 번들은 로컬에 보관합니다.
