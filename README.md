...
> 현재 릴리스: **0.2.0**. 빠른 사용법은 [docs/USER_GUIDE_0.2.md](docs/USER_GUIDE_0.2.md), 설계·연구·라이선스 근거는 [docs/RESEARCH_AND_LICENSES.md](docs/RESEARCH_AND_LICENSES.md)를 먼저 보세요.
...
- 🔌 **플러그인** — 붙였다 떼도 메모리 누수 0 (리스너·타이머·명령·모듈 캐시 전부 자동 정리, 50회 부착/분리 테스트 통과)
...
- 🎙️ **PC·Android 음성 입력** — 설정 가능한 `미스터 로봇` 호출어, 5초 claim ID와 PC 우선 유예로 중복 실행 방지
- 📤 **토큰 없는 파일 전송** — 공유함 또는 등록 작업 폴더에 HTTP 스트리밍 업로드/다운로드, Tailscale 플러그인으로 외부망 사용
...
결과는 `release/Mr.Robot-Setup-0.2.0-x64.exe`입니다. 현재 빌드는 개발용 미서명 설치 파일이므로 조직의 Windows 애플리케이션 제어 정책에서 차단될 수 있습니다. 공개 배포본에는 코드 서명 인증서를 적용해야 합니다.

설치 후 처음 실행하면 **외부 도구 및 의존성 마법사 v2**가 열립니다. Node.js LTS, Git for Windows, Codex CLI, Claude Code, Tailscale, Docker Desktop을 실제 명령으로 검사하고 누락된 항목만 자동 설치합니다. Orca와 Ollama는 선택 기능이므로 강제 설치하지 않습니다. 완료 후에도 설정 → 외부 도구에서 다시 검사하거나 설치할 수 있습니다. Codex·Claude 계정 로그인은 자격 증명을 앱에 복사하지 않고 각 공식 CLI에서 직접 진행합니다.
...
완성 APK는 `release/mobile/Mr.Robot-Mobile-0.2.0.apk`입니다. 같은 Wi-Fi에서는 자동 표시되는 LAN 주소를, 외부망에서는 두 기기에 Tailscale 로그인 후 PC의 `100.x.x.x` 주소를 사용합니다. IP를 수동 추측할 필요 없이 PC의 모바일 연결 화면에 표시되는 주소/QR을 우선 사용하세요.
...
- 기본적으로 **같은 LAN**에서만 접속 가능 (설정에서 변경 가능)
- 새 QR에는 관리자 시크릿이 없고 PIN 정보만 포함됩니다. PIN 교환마다 해시 저장되는 **기기별 폐기 가능 토큰**을 발급합니다.
- 각 기기는 읽기 전용/승인/작업 폴더/전체 권한 상한을 별도로 가지며, PC에서 한 기기만 즉시 연결 해제할 수 있습니다.
- PIN 교환은 1분 5회 제한, 실패 시 5분 잠금
- 직접 PC 조작 RPC는 `전체 허용` 모드가 아니면 차단되고, 일반 작업은 에이전트 도구 권한 정책을 통과
- 외부(다른 네트워크) 접속은 Tailscale/ngrok 등 터널로 포트를 노출하면 됩니다 — 토큰 인증은 항상 유지됩니다
...
- **폰에서 접속 불가**: PC와 폰이 같은 Wi-Fi인지, Windows 방화벽에서 Node 허용인지 확인. (첫 실행 시 방화벽 허용 팝업 확인)
- **LAN IP가 가상 어댑터로 잡힐 때**: 에이전트가 Wi-Fi/이더넷 우선으로 자동 선택합니다. VPN/WSL 가상망은 제외됩니다.
