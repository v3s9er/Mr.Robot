...
export function RunActivityPanel({ phase, activity = [], startedAt, busy, fallback }: {
  ...
  return <details className={`run-panel phase-${state}`}>
    <summary><span className="run-panel-indicator" aria-hidden="true">{state === 'completed' ? '✓' : ['failed', 'cancelled'].includes(state) ? '!' : state === 'approval' ? '◇' : '✦'}</span>
      <span className="run-panel-heading"><b role="status">{LABELS[state]}</b><small>{activity.length ? `도구 ${count}/${activity.length} 완료` : fallback || '진행 상태를 확인하고 있습니다'}{startedAt ? ` · ${Math.floor(elapsed / 60)}:${String(elapsed % 60).padStart(2, '0')}` : ''}</small></span><span className="run-panel-chevron">⌄</span>
    </summary>
